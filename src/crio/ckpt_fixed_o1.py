    ...
    **parent** never returns to Python’s `finally:` code at all because it calls
    ...
    try:
        ...
        if pid == 0:  # Child process
            am_child = True
            try:
                # Clean up lock in child process (child doesn't need parent's locked FD)
                if lock_fd is not None:
                    os.close(lock_fd)
                    lock_fd = None

                # Yield control back to user code (imports etc. in context manager body)
                yield

                # Stop ourselves so the parent can dump (checkpoint)
                os.kill(os.getpid(), signal.SIGSTOP)
                # Once resumed, just continue normal Python flow...
            except Exception as e:
                print(f"Child process error: {e}")
                os._exit(1)
            finally:
                # Don't kill this process after the checkpoint, let normal flow continue
                pass  # (-> return) If we `os._exit(0)` here it bails upon checkpointing
        else:  # Parent process
            child_pid = pid
            try:
